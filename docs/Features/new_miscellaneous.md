- Miscellaneous changes:
    ...
    - Verbs can be declared `protected` or `private` (new `p`/`h` letters in the perms string, alongside `r`/`w`/`x`/`d`). A `protected` verb is only callable by code whose own verb is defined on the same object or a descendant of it (`isa`-style); a `private` verb is only callable by code whose own verb is defined on that exact object (a subclass inheriting, but not overriding, a private verb does not gain access to it). A disallowed call fails with `E_VERBNF`, the same as an unset `x` bit, so the verb behaves as though it doesn't exist. Wizards bypass the restriction; the verb's owner does not. `pass()` is exempt, since it continues the same verb invocation into a parent rather than making an external call
