...
- The shared empty-map singleton (`[]`) is now protected against the same premature-free/corruption crash class the emptylist bandaid exists to prevent. Unlike the empty list, it previously had no such protection at all.
...
- Fixed several inconsistencies found in a pre-merge review of the codepoint-indexing work below, all within this same in-progress version: the `in` operator (`x in s`) returned a raw byte position instead of a codepoint index like `index()`; `pcre_match()` returned a bogus position instead of the documented `{0, -1}` sentinel for a capture group that didn't participate in the match; `pcre_replace()` leaked the compiled pattern whenever asked to replace within an invalid-UTF8 subject; a backspace after a run of bytes that merely fall in the UTF-8 continuation-byte range without forming an actual valid character (e.g. from a non-UTF-8 legacy client) erased more than one byte; and `strtr()`'s `from`/`to` matching had regressed to O(n\*m) from its historical O(n+m).
- `pcre_replace()`'s output sanitizer now also blanks dangerous Unicode format/control characters (C1 controls, zero-width and bidi-override characters like U+202E RIGHT-TO-LEFT OVERRIDE, BOM) encoded as valid multi-byte UTF-8, not just single-byte ASCII control characters.
- `match()`/`rmatch()` no longer report a bogus, out-of-range codepoint position for the end of a zero-width match at the very start of the subject (e.g. pattern `"^"`); this fed a byte-offset underflow into the codepoint converter, and `substitute()` would then copy the rest of the subject into `%0`/`%1`-`%9` instead of nothing.
...
- **Behavior change**: `length()` and every form of string indexing (`str[i]`, `str[i..j]`, both read and write), `index()`, `rindex()`, `strfindall()`, `explode()`, `reverse()`, `pad()`, `strtrim()`/`strtriml()`/`strtrimr()`, `strtr()`, `match()`/`rmatch()`/`substitute()`, and `pcre_match()`/`pcre_replace()` now count and address Unicode codepoints (characters) instead of raw bytes, so a string containing multi-byte UTF-8 text reports and indexes the same way a reader would count it, matching the behavior of `length()`/indexing in most other modern languages. This applies unconditionally, with no server option to opt out. A byte or byte sequence that isn't valid UTF-8 counts as its own one-byte "character", so a string that isn't valid multi-byte UTF-8 behaves exactly as it always did; `value_bytes()` and quota accounting are unaffected and remain byte-based. `pcre_match()`/`pcre_replace()` now compile and match all patterns as UTF-8 (with Unicode property classes like `\p{L}` and `\X` enabled when the server's PCRE2 library supports them), and raise the new `E_INVUTF8` error rather than silently falling back to byte-wise matching when asked to match a subject that isn't valid UTF-8.
- To be explicit about the scope of the above: these are Unicode *codepoints*, not grapheme clusters -- a base letter followed by a combining mark (or a multi-codepoint sequence joined with zero-width-joiner characters, as some emoji are) counts as multiple characters, matching what every codepoint-based language does. No Unicode normalization (NFC/NFD) is performed anywhere, and the Unicode case folding used throughout this release (see below) is *simple* case folding only, with no per-locale exceptions (e.g. Turkish dotted/dotless I do not cross-fold to their ASCII look-alikes).
- Add a `unichr(codepoint, ...)` function, the Unicode-aware companion to `chr()`: each argument is a Unicode codepoint (not a raw byte), encoded as 1-4 UTF-8 bytes and concatenated, so verb code can build non-ASCII text directly instead of chaining multiple single-byte `chr()` calls. As with `chr()`, non-wizards can't produce control characters.
- Add the `E_INVUTF8` error, raised by `pcre_match()`/`pcre_replace()` (see above) when a subject string isn't valid UTF-8.
- MOOcode identifiers -- local variable names, and static `obj.name`/`obj:name(...)` property/verb syntax -- now accept Unicode letters, classified via real Unicode `XID_Start`/`XID_Continue` properties (e.g. `café = 5;` and `#123.café`/`#123:café(...)` are now valid syntax; a symbol like `☃` still isn't, since it isn't a letter). This was already possible indirectly via `obj.(expr)`/`obj:(expr)(args)`/`add_property()`/`add_verb()`, which accept arbitrary UTF-8 names; what's new is the shorter static syntax, and the decompiler (`verb_code()`) printing such names back in that bare form instead of the parenthesized dynamic form. MOOcode keywords and reserved words are unaffected and remain ASCII-only.
- **Behavior change**: verb and property name matching, along with local variable/loop-label lookup, is now case-insensitive for Unicode letters too (e.g. a verb named `café` can be called as `CAFÉ`), not just ASCII as before. Any existing database that already has two non-ASCII verb or property names differing only by case (possible before this release only via the dynamic `add_verb()`/`add_property()` API) will see those names collide under the new case-insensitive fold, with definition order deciding which one is found; this applies to every name-based verb/property operation (call dispatch, `verb_code()`, `set_verb_code()`, `property_info()`, etc.), not just calling.
- `strupper()`/`strlower()` are now Unicode-aware (simple Unicode case mapping, e.g. `strupper("café")` returns `"CAFÉ"`), instead of only affecting ASCII letters. This intentionally excludes locale-specific casing (e.g. Turkish dotless i) and multi-character case expansions (e.g. German `ß` to `"SS"`); a byte or byte sequence that isn't valid UTF-8 passes through unchanged, consistent with the rest of this string work.
- **Behavior change**: general case-insensitive string comparison is now Unicode-case-fold-aware too, extending the verb/property dispatch fold above to every other place case-insensitive string comparison happens -- the `in` operator against a list, the `<`/`<=`/`>`/`>=` string comparison operators (which have always been case-insensitive), and builtins that take an explicit `case-matters` argument such as `listremove_all()`. For example, the Kelvin sign (U+212A, case-folds to ASCII `k`) now compares equal to `"k"` under case-insensitive comparison, consistent with how it already dispatches to a verb or property named with it.
- **Behavior change**: extends the Unicode case-fold above to case-insensitive substring search and sorting too -- `index()`, `rindex()`, `strfindall()`, `strsub()`, and `sort()`'s default (non-`natural`) string ordering. A fold-equivalent match can span a different number of bytes than the search string itself (again, the Kelvin sign vs. `"k"`); `strfindall()`'s position tracking now follows the actual matched span rather than assuming it's always the search string's own length.
- Codepoint-index-to-byte-offset conversion (behind `length()`/string indexing/`index()`/`rindex()`/`strfindall()`/`match()`/`rmatch()`/`pcre_match()`/`pcre_replace()`) is now O(1) for pure-ASCII strings, and amortized O(1) per call for sequential forward access on any string (e.g. `for i in [1..length(s)] ... s[i] ... endfor`), instead of always rescanning from the start of the string. Any other access pattern is unaffected and behaves exactly as before; this is a pure performance change with no observable behavior difference.

**WARNING**: This version increments the database version (DBV_BiFuncId16, DBV_Utf8Errors), making databases incompatible with previous releases.
