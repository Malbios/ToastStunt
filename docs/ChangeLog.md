...
- A compiler warning's diagnostic string now starts its message body with `Warning: ` (e.g. `Line 5:  Warning: Assignment used as a condition; did you mean `=='?`), distinguishing it from a real compile error in the same flat list `set_verb_code()`/`.program` returns - previously both used an identical, unmarked message shape.
...
- Add `protected`/`private` verb visibility: two new letters in the perms string (`p` = protected, `h` = private), settable via `add_verb()`/`set_verb_info()` and readable via `verb_info()`, alongside the existing `r`/`w`/`x`/`d`. A `protected` verb can only be called (`obj:verb-name(...)`) by code whose own verb is defined on the same object or a descendant of it; a `private` verb can only be called by code whose own verb is defined on that *exact* object (inheriting, not overriding, a private verb does not grant access to it). Both fail the same way an unset `x` bit already does -- `E_VERBNF`, not `E_PERM` -- so a disallowed caller sees the verb as not existing at all. Wizards bypass the restriction (matching the existing r/w/x/d wizard-bypass precedent); the verb's own owner does not. `pass()` is unaffected, since it continues the same logical verb invocation into a parent rather than making an external call. No database version bump is required. This closes a common pattern in ToastCore-derived databases of hand-rolling `if (caller != this) return E_PERM; endif` at the top of helper-verb bodies.
